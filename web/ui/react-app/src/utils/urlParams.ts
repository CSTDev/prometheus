...
export function decodePanelOptionsFromQueryString(
  query: string
): { key: string; options: PanelOptions }[] {
  if (query === '') {
    return [];
  }
...
export function encodePanelOptionsToQueryString(
  panels: { key: string; options: PanelOptions }[]
): string {
