...
class Graph extends PureComponent<GraphProps> {
  ...
  renderLabels(labels: { [key: string]: string }) {
    ...
    for (const label in labels) {
      if (label !== '__name__') {
        labelStrings.push(
          '<strong>' + label + '</strong>: ' + this.escapeHTML(labels[label])
        );
      }
    }
    ...
    return {
      ...
      tooltip: {
        ...
        content: (label: string, xval: number, yval: number, flotItem: any) => {
          ...
          const date =
            '<span class="date">' + new Date(xval).toUTCString() + '</span>';
          const swatch =
          ...
          const content =
            swatch +
            (series.labels.__name__ || 'value') +
            ': <strong>' +
            yval +
            '</strong>';
          return (
            date + '<br>' + content + '<br>' + this.renderLabels(series.labels)
          );
        },
        defaultTheme: false,
        lines: true
  ...
  getData() {
    ...
    return this.props.data.result.map((
      ts: any /* TODO: Type this*/,
      index: number
    ) => {
      ...
      for (
        let t = params.startTime;
        t <= params.endTime;
        t += params.resolution
      ) {
        // Allow for floating point inaccuracy.
        if (
          ts.values.length > pos &&
          ts.values[pos][0] < t + params.resolution / 100
        ) {
          data.push([
            ts.values[pos][0] * 1000,
            this.parseValue(ts.values[pos][1])
          ]);
          pos++;
        } else {
  ...
  render() {
    ...
    if (this.props.data.resultType !== 'matrix') {
      return (
        <Alert color="danger">
          Query result is of wrong type '{this.props.data.resultType}', should
          be 'matrix' (range vector).
        </Alert>
      );
    }
