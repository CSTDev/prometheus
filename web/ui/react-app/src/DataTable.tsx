...
class DataTable extends PureComponent<QueryResult> {
  limitSeries(
    series: InstantSample[] | RangeSamples[]
  ): InstantSample[] | RangeSamples[] {
  ...
  render() {
    ...
    switch (data.resultType) {
      case 'vector':
        ...
        break;
      case 'matrix':
        rows = (this.limitSeries(data.result) as RangeSamples[]).map(
          (s, index) => {
    ...
    return (
      <>
        {limited && (
          <Alert color="danger">
            <strong>Warning:</strong> Fetched {data.result.length} metrics, only
            displaying first {rows.length}.
          </Alert>
        )}
