    ...
    fetch('../../api/v1/label/__name__/values', { cache: 'no-store' })
      .then(resp => {
        if (resp.ok) {
          ...
          throw new Error(
            'Unexpected response status when fetching metric names: ' +
              resp.statusText
          ); // TODO extract error
        }
      })
  ...
  render() {
    return (
      <>
        <Row>
          <Col>
            {this.state.timeDriftError && (
              <Alert color="danger">
                <strong>Warning:</strong> Error fetching server time:{' '}
                {this.state.timeDriftError}
              </Alert>
            )}
          </Col>
        ...
        <Row>
          <Col>
            {this.state.fetchMetricsError && (
              <Alert color="danger">
                <strong>Warning:</strong> Error fetching metrics list:{' '}
                {this.state.fetchMetricsError}
              </Alert>
            )}
          </Col>
        ...
        {this.state.panels.map(p => (
          <Panel
            key={p.key}
            options={p.options}
            onOptionsChanged={(opts: PanelOptions) =>
